"""
Planner Agent With Approval - Orchestrates MCP Servers using Strands Framework with human approval workflow
...
MCP_QUERY_URL = os.environ.get('MCP_QUERY_URL', 'https://dltsnftnbwnpbqcea3lm4ljdlu0uaaia.lambda-url.us-east-1.on.aws/')
MCP_EVAL_URL = os.environ.get('MCP_EVAL_URL', 'https://alq6pflsaerscwzvkxqlyonfki0tzytu.lambda-url.us-east-1.on.aws/')
MCP_GITHUB_URL = os.environ.get('MCP_GITHUB_URL', 'https://dlxagky44bv6odorc6763yswfa0qnbhp.lambda-url.us-east-1.on.aws/')
...
class PlannerAgentWithApproval:
    """Agent that orchestrates MCP servers with autonomous reasoning and human approval"""
    
    def __init__(self, debug: bool = False, custom_prompt: str = None, approval_required: bool = True):
        self.debug = debug
    ...
    def process_pending_approvals(self):
        """Process any approved requests that haven't been executed"""
        approved_requests = self.approval_store.get_approved_requests()
        
        if not approved_requests:
            if self.debug:
                print("[Agent] No approved requests to process")
            return
        
        if self.debug:
            print(f"[Agent] Processing {len(approved_requests)} approved requests...")
        
        for request in approved_requests:
            if request['action'] == 'submit_prompt_update':
                if self.debug:
                    print(f"[Agent] Executing approved PR creation: {request['id']}")
                    
                # Execute the PR creation
                result = self.github_tool(
                    prompt_text=request['prompt_text'],
                    reason=request['reason']
                )
                
                if result.get('success'):
                    print(f"✓ Created PR from approved request {request['id']}: {result.get('pr_url')}")
                    # Mark the request as processed
                    self.approval_store.mark_as_processed(request['id'])
                else:
                    print(f"❌ Error executing approved request {request['id']}: {result.get('error')}")
    
    def _create_pr_approval_request(self, prompt_text: str, reason: str) -> str:
        """Create an approval request for PR creation"""
        request_data = {
        ...
        request_id = self.approval_store.create_request(request_data)
        
        if self.debug:
            print(f"[Agent] Created PR approval request: {request_id}")
    ...
    def execute_goal(self, goal: str, stream: bool = True, custom_prompt_override: str = None) -> str:
        ...
        # First process any pending approvals
...
Always aim to be helpful, informative, and user-friendly."""
                ...
                if self.approval_required:
                    # Create an approval request instead of direct PR
                    request_id = self._create_pr_approval_request(prompt_text, reason)
                    result_msg = f"⏳ PR creation requires approval (ID: {request_id})"
                    if stream:
                        print(result_msg)
                        print(f"Run `python approve_action.py --approve {request_id}` to approve")
                    return result_msg
                else:
                    # Direct PR creation without approval
...
Always aim to be helpful, informative, and user-friendly."""
                        ...
                        if self.approval_required:
                            ...
                            pr_msg = f"\n⏳ PR creation requires approval (ID: {request_id})"
                            pr_msg += f"\nReason: {reason}"
                            pr_msg += f"\nRun `python approve_action.py --approve {request_id}` to approve"
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Planner Agent with Approval - MCP Server Orchestrator')
    ...
    parser.add_argument('--no-approval', action='store_true', help='Skip approval for actions (not recommended for production)')
    ...
    # Initialize agent
    agent = PlannerAgentWithApproval(
        debug=args.debug,
        custom_prompt=custom_prompt,
        approval_required=not args.no_approval
    )
    
    # Process any existing approved requests
    agent.process_pending_approvals()
    ...
    if args.goal:
        ...
        # Interactive mode
        print("=" * 60)
        print("Planner Agent with Approval - Interactive Mode")
        print("=" * 60)
        print("\nExamples:")
        ...
        print("\nNOTE: Actions that modify the system will require approval.")
        print("\nType 'exit' to quit\n")
        ...
        while True:
            try:
                ...
                agent.execute_goal(goal, stream=True)
                print("\n" + "-" * 60)
